...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Foo":
		...
		case "EchoLength":
			...
			return (*Foo).EchoLength(&parent, message), nil
		case "Big":
			var parent Foo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Foo).Big(&parent, ctx, message, verbose), nil
		case "GrepDir":
			var parent Foo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Foo).GrepDir(&parent, ctx, directory, pattern)
		case "Bundle":
			var parent Foo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Foo).Bundle(&parent, ctx, message), nil
		case "Search":
			var parent Foo
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Foo).Search(&parent, ctx, message), nil
		default:
				...
				dag.TypeDef().WithObject("Foo", TypeDefWithObjectOpts{Description: "Functions for working with Foo."}).
					WithFunction(
						dag.Function("Base",
							...
							WithDescription("Returns a base Container with some packages installed.")).
					WithFunction(
						dag.Function("EchoLength",
							dag.TypeDef().WithObject("Container")).
							WithDescription("Returns a container that echoes a message and its length using a Bash script.").
							WithArg("message", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("Big",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Returns a message as a large banner and optionally provides metadata.").
							WithArg("message", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("\"Hello, Dagger!\"")}).
							WithArg("verbose", dag.TypeDef().WithKind(BooleanKind).WithOptional(true), FunctionWithArgOpts{DefaultValue: JSON("false")})).
					WithFunction(
						dag.Function("GrepDir",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Runs grep using pattern in a container over provided Directory.").
							WithArg("directory", dag.TypeDef().WithObject("Directory")).
							WithArg("pattern", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("Bundle",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Takes a message and creates a Directory with a banner and metadata Files.").
							WithArg("message", dag.TypeDef().WithKind(StringKind))).
					WithFunction(
						dag.Function("Search",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Searches over a Directory of files created using message returning result.").
							WithArg("message", dag.TypeDef().WithKind(StringKind)))), nil
	default:
